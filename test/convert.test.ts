...
describe('Anthropic ↔ AI SDK Conversion', () => {
  describe('convertAnthropicToCoreMessages', () => {
    it('should convert simple text messages', () => {
      ...
      const coreMessages = convertAnthropicToCoreMessages(anthropicRequest);

      expect(coreMessages).toHaveLength(3);
      expect(coreMessages[0].role).toBe('user');
      expect(coreMessages[0].content).toEqual([{ type: 'text', text: 'Hello!' }]);
      expect(coreMessages[1].role).toBe('assistant');
      expect(coreMessages[1].content).toEqual([{ type: 'text', text: 'Hi there!' }]);
    });
    ...
    it('should convert structured content with images', () => {
      ...
      expect(coreMessages[0].content[0]).toEqual({ type: 'text', text: 'What do you see?' });
  ...
  describe('convertCoreToAnthropicResponse', () => {
    it('should convert simple text response', () => {
      ...
      const anthropicResponse = convertCoreToAnthropicResponse(
        coreOutput,
  ...
  describe('convertAnthropicTool', () => {
    it('should convert valid Anthropic tools to AI SDK format', () => {
      ...
      const aiSdkTools = convertAnthropicTool(anthropicTools);
    ...
    it('should convert all valid tools including system tools', () => {
      ...
      expect(result[2].function.description).toBe('Anthropic text_editor_20250124 tool');
      expect(result[2].function.parameters.properties).toEqual({});
    });

    it('should handle non-array input gracefully', () => {
      const result = convertAnthropicTool(null as any);
      expect(result).toEqual([]);

      const result2 = convertAnthropicTool('not an array' as any);
  ...
  describe('Schema Validation', () => {
    ...
    it('should reject invalid Anthropic responses', () => {
      const invalidResponses = [
        ...
        { id: 'msg_123' },
        // Wrong type
        { id: 'msg_123', type: 'completion', role: 'assistant', content: [], model: 'test', stop_reason: 'end_turn', stop_sequence: null, usage: { input_tokens: 0, output_tokens: 0 } },
        // Wrong role
        { id: 'msg_123', type: 'message', role: 'user', content: [], model: 'test', stop_reason: 'end_turn', stop_sequence: null, usage: { input_tokens: 0, output_tokens: 0 } },
        // Invalid stop_reason
        { id: 'msg_123', type: 'message', role: 'assistant', content: [], model: 'test', stop_reason: 'invalid_reason', stop_sequence: null, usage: { input_tokens: 0, output_tokens: 0 } },
        // Negative token counts
        { id: 'msg_123', type: 'message', role: 'assistant', content: [], model: 'test', stop_reason: 'end_turn', stop_sequence: null, usage: { input_tokens: -1, output_tokens: 0 } }
      ];
  ...
  describe('End-to-End Conversion', () => {
    it('should handle complete request-response cycle', () => {
      ...
      const coreMessages = convertAnthropicToCoreMessages(request);
      expect(coreMessages).toHaveLength(1);
